...
Design and implement a class representing a basic vending machine capable of keeping track of the number of items of each type currently in the machine, the amount of change currently in the machine for each type of coin, and to return correct change given a managedProduct selection and a set of coins submitted. The solution is expected to be in the form of two interfaces (see below) and a class implementing these interfaces, along with any helpers etc that you may wish to create.

Each vending machine instance has a fixed number of managedProduct slots, and supports a fixed number of coin types (both set at instantiation time), but can have price and quantity for each managedProduct slot adjusted as well we the amount of change available - imagine an operator collecting money from the machine, filling it up with new items and potentially changing which products are being sold.

The vending machine also supports price display and actual purchases of products using coins provided by a user. This operation is expected to work the way a vending machine would, i.e. buying a managedProduct decreases the inventory for that managedProduct by one, and the amount of change in the machine is updated.

Please focus on the vending machine itself rather than a rich domain around products. It is perfectly fine to represent a managedProduct with simply a string or number. Also note that the spec is intentionally vague and all method signatures or names are for illustration purposes only, please select method names, method signatures etc to your liking, as long as the implementation supports the behaviour outlined.

Constructing a vending machine instance

The vending machine class is instantiated with a set of managedProduct slots and a set of supported coin types. While the number of available products per slot, managedProduct price, and amount of change available can all change (see below), the set of managedProduct slots and supported coin types should be seen as immutable for any given instance of the vending machine. An example constructor invocation may be new VendingMachine(10, Arrays.asList(0.10, 0.20, 0.50, 1.0))

… to support 10 managedProduct slots (implicitly identified as 0..9), and coins worth 10p, 20p, 50p and £1, but you are free to choose the types as you see fit.
...
Set/get the number of items for a given managedProduct slot

Set/get the price per managedProduct for a given managedProduct slot

Set/get the number of coins available for a given type of coin

The implementation should be thread-safe and throw an appropriate exception for operations that do not make sense, e.g. IllegalStateException if setting the number of items for a managedProduct slot with no price specified, or IllegalArgumentException if trying to set the price for a managedProduct slot that the machine does not have.
...
Get the price per managedProduct for a given managedProduct slot

Buy the managedProduct for a given managedProduct slot. This should be one operation that accepts a managedProduct slot identifier, a collection of coins, and returns another collection of coins representing the change given if successful, e.g:

buyProduct(1, Arrays.asList(1.0, 1.0)) might return an array of {0.5, 0.2, 0.1} if the managedProduct costs £1.30

The implementation should throw IllegalStateException if the current state of the machine does not support the operation (e.g. the managedProduct is sold out, the managedProduct doesn’t have a price), and should throw IllegalArgumentException if the parameters are invalid (non-existing managedProduct slot referenced, not enough money provided).
