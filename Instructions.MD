...
Each vending machine instance has a fixed number of product slots, and supports a fixed number of coin types (both set at instantiation time), but can have price and quantity for each product slot adjusted as well we the amount of change available - imagine an operator collecting money from the machine, filling it up with new items and potentially changing which products are being sold.
...
Set/get the number of items for a given product slot

Set/get the price per item for a given product slot

Set/get the number of coins available for a given type of coin

The implementation should be thread-safe and throw an appropriate exception for operations that do not make sense, e.g. IllegalStateException if setting the number of items for a product slot with no price specified, or IllegalArgumentException if trying to set the price for a product slot that the machine does not have.
...
buyProduct(1, Arrays.asList(1.0, 1.0)) might return an array of {0.5, 0.2, 0.1} if the product costs £1.30
